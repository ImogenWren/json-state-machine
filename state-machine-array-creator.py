"""
Cript to quickly create the correct state machine array given the list of states as enum and the list of prototype states

each list can be copied from source code directly and script run to generate state machine array as output.
...
states = []
for match in re.findall(r"\bSTATE_[A-Z0-9_]+\b", enum_clean):
    if match != "STATE_NUM_STATES" and match != "NUM_STATES":
        states.append(match)
...
for match in re.findall(r"\bsm_state_[a-z0-9_]+\b", func_clean):
    functions[normalize(match.replace("sm_state_", ""))] = match

# -----------------------------
# Match and emit table
# -----------------------------
