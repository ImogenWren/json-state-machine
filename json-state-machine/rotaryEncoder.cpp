...
void rotaryEncodeSensor::begin() {
  ...
  //EIC->CONFIG[0].D = 1;
 // NVIC_SetPriority(SysTick_IRQn, 0);  // Give SysTick a high priority
 // NVIC_SetPriority(EIC_IRQn, 4);      // Lower priority for external interrupt
...
int16_t rotaryEncodeSensor::getEncoderPos() {
  return encoderPos;
}

float rotaryEncodeSensor::convertPosDeg() {
  encoderDeg = (360.0 / float(pulses_per_revolution)) * float(encoderPos);
  return encoderDeg;
}

void rotaryEncodeSensor::convertPosRad() {
  encoderRad = ((2 * PI) / float(pulses_per_revolution)) * float(encoderPos);
}

void rotaryEncodeSensor::calibrate_position(int16_t currentPos){
  encoderPos = currentPos;
}
...
void rotaryEncodeSensor::encoderWrap(void) {
  if (encoderPos > pulses_per_revolution) {
    encoderPos -= pulses_per_revolution;
...
void rotaryEncodeSensor::EIC_handle() {
#ifdef ARDUINO_ARCH_SAMD
  if (EIC->INTFLAG.bit.EXTINT9) {  // Check if interrupt was triggered on EXTINT5
    // Handle the encoder logic here
    // Serial.println("9 triggered");
    EIC->INTFLAG.reg = (9 << 1);   // Clear the interrupt flag for EXTINT5
    EIC->INTFLAG.bit.EXTINT9 = 1;  // Clear interrupt flag for EXTINT5
  }

  if (EIC->INTFLAG.bit.EXTINT3) {  // Check if interrupt was triggered on EXTINT5
                                   // Handle the encoder logic here
                                   // Serial.println("3 triggered");
    EIC->INTFLAG.reg = (3 << 1);   // Clear the interrupt flag for EXTINT5
    EIC->INTFLAG.bit.EXTINT9 = 1;  // Clear interrupt flag for EXTINT5
  }
#endif
}
